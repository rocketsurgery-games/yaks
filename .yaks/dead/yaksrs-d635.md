...
Obviated by yaksrs-2956: we're dropping edtui's syntect highlighting entirely (coloring both detail + editor with a hand-rolled highlighter via edtui's public state.highlights API), so yaks no longer needs a fancy-regex edtui. Slaughtering. Could still be offered upstream as a courtesy for other edtui users, but that's not yaks work.
