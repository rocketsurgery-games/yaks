...
Phase 1 complete. Delivered with parity + tests: full write path (create/update/--note/--title, status moves, dep, reparent), query surface (list/next/tangled/search/stats/show, filtering), rollup, --json everywhere, golden snapshot harness (35 tests), npm distribution launcher, startup bench (~6ms vs ~45ms), and a schema-version gate. Deferred to top-level: yaksrs-8f50 (rkyv index — premature at current scale) and yaksrs-a49c (attach/detach). Dropped yaksrs-35b6 (CI interop test) — will iterate Rust-side and retire Python once comfortable.
